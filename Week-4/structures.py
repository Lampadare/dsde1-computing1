...
def part_reverse(the_list, beginning, end):
    if end < beginning :
        ...
        raise(ValueError)
    elif (beginning or end) < 0:
        raise(ValueError)
    else:
        second_list = the_list[beginning:end]
        second_list.reverse()
        return second_list
...
def palindrome_sentence(sentence):
    sentence_first = sentence.lower()
    sentence_second = ''.join(filter(str.isalnum, sentence_first)) 
    sentence_one = list(sentence_second)
    sentence_two = list(sentence_second)
    sentence_two.reverse()
